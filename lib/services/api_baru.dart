  ...
  class ApiBaru {
    ...
    // --------------------------
    // Order (multipart) - DENGAN BUKTI PEMBAYARAN (Transfer Bank)
    // Backend: POST /api/v1/orders (multipart/form-data)
    // Response: { "status": "success", "order_id": 123, "total": 120000 }
    // --------------------------
    static Future<Map<String, dynamic>> createOrder({
      ...
      if (reservationTableNumber != null && reservationTableNumber.isNotEmpty) {
      ...
      if (reservationGuestCount != null && reservationGuestCount.isNotEmpty) {
      ...
      try {
        ...
        if (resp.statusCode == 201 || resp.statusCode == 200) {
          // Go response: { "status": "success", "message": "...", "order_id": 123, "total": 120000 }
          return responseData;
        } else {
          throw Exception(
            responseData['detail'] ??
    ...
    // --------------------------
    // Order MIDTRANS - multipart TANPA file bukti pembayaran
    // Backend: POST /api/v1/orders (multipart/form-data, tanpa file)
    // CATATAN: Go handler wajib multipart, jadi kita kirim multipart tanpa file
    // Response: { "status": "success", "order_id": 123, "total": 120000 }
    // --------------------------
    static Future<Map<String, dynamic>> createOrderMidtrans({
      ...
      // ⚠️ PENTING: Go handler butuh multipart, tapi Midtrans tidak ada file.
      // Kita kirim multipart dengan field saja (tanpa file attachment).
      // Pastikan di Go handler, cek file bukti_pembayaran dibuat optional
      // saat payment_method == "midtrans". Lihat catatan di bawah.
      final url = Uri.parse('$baseUrl/orders');
      final request = http.MultipartRequest('POST', url);
      request.headers['Authorization'] = 'Bearer $token';

      request.fields['payment_method'] = 'midtrans';
      request.fields['order_type'] = orderType;
      request.fields['items_data'] = itemsDataJson;
      ...
      // Tidak ada file — Midtrans tidak butuh bukti pembayaran manual
      ...
      try {
        ...
        if (resp.statusCode == 201 || resp.statusCode == 200) {
          // Go response: { "status": "success", "order_id": 123, "total": 120000 }
          return responseData;
        } else {
          throw Exception(
            responseData['detail'] ??
    ...
    // --------------------------
    // MIDTRANS SNAP TOKEN
    // Backend: POST /api/v1/payments (JSON)
    // Body: { "order_id": 123 }
    // Response: { "snap_token": "xxxx-xxxx-xxxx" }
    // --------------------------
    static Future<Map<String, dynamic>> createMidtransSnap({
      ...
      // ✅ FIX: /payments (plural) sesuai route Go: api.POST("/payments", ...)
      ...
      try {
        ...
        if (response.statusCode == 200) {
          // Go response: { "snap_token": "xxxx" }
          return jsonDecode(response.body);
        } else {
          final body = jsonDecode(response.body);
          throw Exception(
            body['error'] ??
                'Gagal membuat Snap Midtrans (${response.statusCode})',
          );
        }
      } catch (e) {
    ...
    /// Backend route: GET /api/v1/orders/:order_id
      ...
      try {
        ...
        if (res.statusCode == 200) {
          ...
          if (body is Map && body['recommendations'] != null) {
            final list = List.from(body['recommendations'])
    ...
    /// Backend: GET /api/v1/recommendations/item/{item_id}
